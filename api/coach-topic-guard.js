...
/** 中文註解：明確離題（數學、程式、一般學科作業等） */
const OFF_TOPIC_PATTERNS = [
  /微積分|calculus|導數|積分|極限|微分|線性代數|矩陣|機率論|統計學作業/iu,
...
function buildOffTopicReply(message) {
  const q = String(message || '').trim().slice(0, 40);
  const preview = q ? `「${q}${message.length > 40 ? '…' : ''}」` : '這類問題';
  return (
    '我是梅娜斯葡萄酒社交教練，專注在葡萄酒、餐桌與社交場合的實戰建議。\n\n' +
    `${preview}超出我的服務範圍，我這裡不解答一般學科或無關話題，避免浪費你的使用額度。\n\n` +
    '你可以改問例如：\n' +
    '• 商務飯局怎麼破冰、敬酒順序\n' +
    '• 約會或聚餐怎麼選酒、怎麼聊\n' +
    '• 某道菜（牛排、麻辣鍋）適合配什麼酒\n\n' +
    '換一個葡萄酒或社交情境的問題，我馬上陪你拆解。'
  );
}
